	  ...
	  $event_id = $_GET['id'];
		...
		//	$password = password_hash($password, PASSWORD_DEFAULT);
			...
			if(!empty($filename)){
				...
				move_uploaded_file($_FILES['image']['tmp_name'], 'assets/images/event/details/'.$new_filename);	
			}
			try{
				$stmt = $conn->prepare("INSERT INTO  tbl_event_details (event_id ,image) VALUES (:event_id, :image)");
				$stmt->execute(['event_id'=>$event_id,'image'=>$new_filename]);
...
<?php
	...
	if(isset($_POST['delete'])){
		...
		try{
			$stmt = $conn->prepare("DELETE FROM tbl_event_details WHERE id=:id");
			$stmt->execute(['id'=>$id]);
            ...
            <div class="col-12">
                <div class="card">
                    ...
                    try{
                      ...
                      $stmt->execute();
                      foreach($stmt as $row){
                        $image = (!empty($row['image'])) ? 'assets/images/event/details/'.$row['image'] : 'assets/images/users/profile.jpg';
...
<?php include 'event-details-popup.php'?>
