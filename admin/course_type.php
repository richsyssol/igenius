...
<?php
	if(isset($_POST['add'])){
		...
		$stmt = $conn->prepare("SELECT *, COUNT(*) AS total FROM tbl_course_type WHERE id=:id");
		$stmt->execute(['id'=>$id]);
		$row = $stmt->fetch();
		...
		//	$password = password_hash($password, PASSWORD_DEFAULT);
			...
			try{
			 $stmt = $conn->prepare("INSERT INTO tbl_course_type (c_id, name, description) VALUES (:c_id, :name, :description)");
				$stmt->execute(['c_id'=>$c_id, 'name'=>$name, 'description'=>$description]);
				$_SESSION['success'] = 'Course Type added successfully';
...
<?php
	if(isset($_POST['delete'])){
		...
		try{
			$stmt = $conn->prepare("DELETE FROM tbl_course_type WHERE id=:id");
			$stmt->execute(['id'=>$id]);

			$_SESSION['success'] = 'Course Type deleted successfully';
		}
		catch(PDOException $e){
			$_SESSION['error'] = $e->getMessage();
		}
