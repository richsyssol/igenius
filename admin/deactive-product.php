                                    ...
                                    <div class="page-title-right">
                                        <ol class="breadcrumb m-0">
                                           <li class="breadcrumb-item"><a href="index.php">Dashboard</a></li>
                                            <li class="breadcrumb-item active">De-active Product</li>
                                        </ol>
                                    </div>
                                    <h4 class="page-title">De-active Product</h4>
                                </div>
                            </div>
                        </div>     
                        <!-- end page title --> 
 ...
 <div class="row">
            <div class="col-12">
                <div class="card">
                                ...
                                <div class="col-sm-12">
                                    <table id="datatable-buttons" class="table table-striped dt-responsive nowrap w-100 dataTable no-footer dtr-inline" role="grid" aria-describedby="datatable-buttons_info" style="width: 1244px;">
                                            <thead>
                                                    ...
                                                    <th class="sorting sorting_asc" tabindex="0" aria-controls="datatable-buttons" rowspan="1" colspan="1" style="width: 199.4px;" aria-sort="ascending" aria-label="Product Id: activate to sort column descending">Product Id</th>
                                        ...
                                        <th class="sorting sorting_asc" tabindex="0" aria-controls="datatable-buttons" rowspan="1" colspan="1" style="width: 199.4px;" aria-sort="ascending" aria-label="Images: activate to sort column descending">Images</th>
                                                    ...
                                                    <th class="sorting" tabindex="0" aria-controls="datatable-buttons" rowspan="1" colspan="1" style="width: 73.4px;" aria-label="contact: activate to sort column ascending"> View </th>
                    ...
                    try{
                      $now = date('Y-m-d');
                      $stmt = $conn->prepare("SELECT products.*,products.id from products  where products.status='Deactive' Order By created_on DESC");
                      $stmt->execute();
                      $cnt=1;
                      foreach($stmt as $row){
                        ...
                        ?>
                                                 ...
                                                 <tr class="odd">
                                                    <td class="dtr-control sorting_1" tabindex="0"><?php echo $cnt++;?></td>                                          <td> <?php echo ($row['product_id']); ?> </td>
                                            ...
                                            $stmt = $conn->prepare("SELECT * from product_images WHERE product_id='".$row['id']."'");
                                            $stmt->execute();
                                            foreach ($stmt as $row2) {
                                           
                                        ?>
                                        <img src="<?php echo $row2['medium_path'];?>" width="20">
                                        <?php
                                        }
                                        ...
                                        <?php
                                            ...
                                            try{
                                            $now = date('Y-m-d');
                                              $stmt = $conn->prepare("SELECT tbl_cat.*,tbl_cat.id,main_category.id as m_id, main_category.main_category as m_main_category from tbl_cat join main_category on main_category.id=tbl_cat.category where tbl_cat.product_id='".$row['id']."'");
                                            $stmt->execute();
                                            $cnt=1;
                                            foreach($stmt as $row1){
                                               
                                        ?>
                                                         <?php echo mb_strimwidth($row1['m_main_category'], 0, 20, "..."); ?>
                                                          <?php
                                              }
                                            }
                                            catch(PDOException $e){
...
<?php include 'deactive-product-popup.php'?>
