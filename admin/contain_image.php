...
<?php
	if(isset($_POST['add'])){
		...
		$stmt = $conn->prepare("SELECT *, COUNT(*) AS total FROM tbl_c_video WHERE id=:id");
		$stmt->execute(['id'=>$id]);
		$row = $stmt->fetch();

		if($row['total'] > 0){
			$_SESSION['error'] = 'type already taken';
		}
		else{
		//	$password = password_hash($password, PASSWORD_DEFAULT);
			...
			try{
			 $stmt = $conn->prepare("INSERT INTO tbl_c_video (c_id, video_url) VALUES (:c_id, :video_url)");
				$stmt->execute(['c_id'=>$c_id, 'video_url'=>$video_url]);
				$_SESSION['success'] = 'Course video added successfully';
...
<?php
	if(isset($_POST['delete'])){
		...
		try{
			$stmt = $conn->prepare("DELETE FROM tbl_c_video WHERE id=:id");
			$stmt->execute(['id'=>$id]);

			$_SESSION['success'] = 'Course Video deleted successfully';
		}
		catch(PDOException $e){
			$_SESSION['error'] = $e->getMessage();
		}
                ...
                ?>
                    <div class="row">
                        <div class="col-lg-6">
                            <div class="card">
                                <div class="card-body">
                                    <h5 class="text-uppercase bg-light p-2 mt-0 mb-3">Course Type List</h5>
                                <table  class="table table-hover small-text">
                                    ...
                                    $id=intval($_GET['id']);
                                    $query=mysqli_query($conn,"SELECT * from tbl_c_video where c_id='$id'");
                            ...
                            <div class="card-body">
                                <h5 class="text-uppercase mt-0 mb-3 bg-light p-2">Course Type Form</h5>
                                  <form  method="post" enctype="multipart/form-data">
                                       ...
                                       <input type="hidden" value="<?php echo $_GET['id'];?>" name="c_id">
                                    <div class="row">
                                         <div class="mb-3">
                                              <label>Enter Video Url</label>
                                            <input type="url" class="form-control" placeholder="Enter Video Url" name="video_url">
                                         </div>
                                    </div>
